...
module Make =
  ...
  let perform =
      (
        ~perform_edit_state: (~delete_whole_selection: bool=?, _, _) => _,
    ...
    and perform_pointing =
      ...
      | Move(d) =>
        Option.map(
          EditState.of_pointing,
          move_pointing(d, (pointing, frame)),
        )

      | Delete(d) =>
        perform_pointing(Mark, pointing, frame)
        |> OptUtil.and_then(perform_edit_state(Move(d)))
        |> OptUtil.and_then(
             perform_edit_state(~delete_whole_selection=true, Delete(d)),
           )
    ...
    and perform_selecting =
      ...
      | Delete(_) =>
        ...
        | _ =>
          ...
          let (prefix, suffix) =
            TupleUtil.map2(
              Selection.to_list(Either.l, t =>
                Either.R([Selection.Tessera(t)])
              ),
              (prefix, suffix),
            );
