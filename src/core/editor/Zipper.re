...
module Base = {
  ...
  let unroll = (~ctx=Ctx.empty, side: Dir.t, cell: Cell.Base.t(_)) => {
    let f_open =
      side == L
        ? ([], Slope.Up.unroll(cell)) : (Slope.Dn.unroll(cell), []);
    Ctx.map_hd(Frame.Open.cat(f_open), ctx);
  };
  let mk_unroll = (~ctx=Ctx.empty, side: Dir.t, cell: Cell.t) =>
    mk(unroll(side, cell, ~ctx));

  let rec unzip = (~ctx=Ctx.empty, cell: Cell.Base.t(_)) => {
    open Options.Syntax;
    let* hd = Option.map(Path.Cursor.hd, cell.marks.cursor);
    let m = Cell.Base.get(cell);
    switch (hd) {
    ...
    | Ok(step) =>
      ...
      | Loop((pre, cell, suf)) =>
        unzip(~ctx=Ctx.add((pre, suf), ctx), cell)
      | Link((pre, tok, suf)) =>
