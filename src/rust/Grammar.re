...
and Exp: SORT = {
  ...
  let exp_with_block = () =>
    [match_exp()] @ loop_exp() @ [if_exp(), block_exp()];
  ...
  let operand = () =>
    alt(
      [
        ...
        seq([c("("), atom(), c(")")]),
        //Arrays
        seq([c("["), atom(), opt(star(seq([c(","), atom()]))), c("]")]),
      ]
      @ exp_with_block(),
    );
